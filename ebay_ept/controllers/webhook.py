...
class Webhook(http.Controller):
    ...
    @http.route('/ebay_partners_delete', type='http', auth="public", methods=['GET'], csrf=False)
    def get_challenge_code_for_check_validations(self, **kwargs):
        ...
        if "challenge_code" in request.httprequest.url:
            parsed = urlparse.urlparse(browser_url)
            challange_code = parse_qs(parsed.query)['challenge_code'][0]
            config_para = request.env['ir.config_parameter']
            verification_token = config_para.sudo().get_param('Marketplace Account Verification Token')
            end_point = config_para.sudo().get_param('web.base.url') + "/ebay_partners_delete"
    ...
    @http.route('/ebay_partners_delete', type='json', auth="public", methods=['POST'], csrf=False)
