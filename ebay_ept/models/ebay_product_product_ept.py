...
class EbayProductProductEpt(models.Model):
    ...
    def update_price_in_ebay(self, instance, ebay_products=False):
        ...
        for ebay_product in ebay_products:
            price = instance.pricelist_id.get_product_price_ept(ebay_product.product_id)
        ...
        self.export_price_stock_in_ebay(price_list_data, instance, log_book_id)

        if not log_book_id.log_lines:
            log_book_id.sudo().unlink()
        return True

    def export_stock_levels_ebay(self, instance, ebay_products, log_book_id=False, ebay_product_stock=False):
        """
        ...
        self.export_price_stock_in_ebay(inv_list_data, instance, log_book_id)
    ...
    def export_price_stock_in_ebay(self, list_data, instance, log_book_id):
        """
        ...
        for inv_list in split_every(4, list_data):
            try:
                ...
                log_line_obj.create_log_lines(error, log_book_id.model_id.id, False, log_book_id)
    ...
    def export_stock_in_ebay(self, instance):
        ...
        if not product_ids:
            message = "No products found in warehouses: %s for Export Stock" % warehouse_ids.name
            log_line_obj.create_log_lines(message, log_book_id.model_id.id, False, log_book_id)
            return False
        ...
        if not ebay_exported_products:
            message = "No Ebay products found in warehouses: %s for Export Stock and site: %s" % (
                warehouse_ids.name, instance.name)
            log_line_obj.create_log_lines(message, log_book_id.model_id.id, False, log_book_id)
            return False
    ...
    def create_ebay_product_stock_log(self, instance_id):
        """
        Create ebay product stock log.
        :param instance_id: ebay instance id
        :return: common log book object
        """
        model_id = self.env['common.log.lines.ept'].get_model_id('ebay.product.product.ept')
        return self.env["common.log.book.ept"].create(
            {'type': 'import', 'module': 'ebay_ept', 'model_id': model_id, 'ebay_instance_id': instance_id})
