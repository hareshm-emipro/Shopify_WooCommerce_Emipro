...
class EbayProductTemplateEpt(models.Model):
    ...
    def prepare_product_dict(self, ebay_product_template, instance, publish_in_ebay, schedule_time):
        ...
        if ebay_product_template.count_total_variants > 1:
            template_description = ebay_product_template.description or ''
            if template_description == '<p><br></p>':
                template_description = ''
            # prod_desc = (html.escape(template_description).encode("utf-8")).decode('iso-8859-1')
            prod_desc = cgi.escape(template_description)
            sub_dict.update({'Description': '<![CDATA[' + prod_desc + ']]>'})
