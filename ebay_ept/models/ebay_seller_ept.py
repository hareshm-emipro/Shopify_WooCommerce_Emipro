...
class EbaySellerEpt(models.Model):
    ...
    def create(self, vals):
        ...
        :param vals: values to create eBay details
        ...
        seller = super(EbaySellerEpt, self).create(vals)
        try:
    ...
    def auto_sync_active_products_listings(self, args=None):
        ...
        if seller_id:
            ...
            if not instances:
                return False
            process_import_export_obj.with_context(is_call_from_cron=True).ebay_sync_import_products(instances)
        return True
    ...
    @api.model
    def generate_varification_token(self):
        """
        Find or create marketplace varification token.
        :return:
        """
        config_para = self.env['ir.config_parameter']

        token_exists = config_para.search([('key', '=', 'Marketplace Account Verification Token')])
        if not token_exists:
            db_uuid = config_para.sudo().get_param('database.uuid')
            randomstr = ''.join(random.choices(string.ascii_letters + string.digits + '_', k=10))
            config_para.sudo().create({
                'key': "Marketplace Account Verification Token", "value": db_uuid + randomstr})
        return True
