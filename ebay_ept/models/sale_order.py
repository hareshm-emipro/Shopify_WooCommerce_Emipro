...
class SaleOrder(models.Model):
    ...
    def create_ebay_order_logs(self, instance):
        """
        ...
        return common_log_book_obj.create({
            'name': 'sales', 'type': 'import', 'module': 'ebay_ept', 'model_id': model_id,
            'res_id': self.id, 'ebay_instance_id': instance_id})
    ...
    def create_ebay_sales_order_ept(self, seller, order_queue_line, log_book_id, order_queue):
        """
        ...
        instance = seller.instance_ids.filtered(lambda x: x.site_id.id == ebay_site.id)
        if instance:
            skip_order = self.ebay_check_mismatch_details_for_order(instance, order_response, log_book_id,
                                                                    order_queue_line)
            if skip_order:
                return False
            ...
            order_lines = self.create_ebay_sale_order_lines(order_response, instance, sale_order, log_book_id,
                                                            order_queue)
            if not order_lines:
                return False
            ...
            log_line = log_lines_obj.create_log_lines(message, log_book_id.model_id.id, False, log_book_id,
                                                      order_ref='order_ref')
    ...
    def ebay_check_mismatch_details_for_order(self, instance, order_response, log_book_id, order_queue_line):
        """
        ...
        """
        skip_order = self.check_payment_details_and_workflow_ept(instance, order_response, log_book_id,
                                                                 order_queue_line)
        if not skip_order:
            skip_order = self.check_product_mismatch_details(instance, order_response, log_book_id,
                                                             order_queue_line)
            if not skip_order:
                skip_order = self.get_ebay_shipping_details(instance, order_response, log_book_id, order_queue_line)

        return skip_order

    def check_payment_details_and_workflow_ept(self, instance, order_response, log_book_id, order_queue_line):
        """
    ...
    def check_product_mismatch_details(self, instance, order_response, log_book_id, order_queue_line):
        """
        ...
        for order_line in order_lines:
            ...
            if not any([ebay_sku, item_id]):
                ...
                order_queue_line.write({'state': 'failed', 'processed_at': datetime.now()})
                return True
            return self.search_create_product_for_order_ept(order_response, item_id, ebay_sku, instance, log_book_id,
                                                            order_queue_line)

    def search_create_product_for_order_ept(self, order_response, item_id, ebay_sku, instance, log_book_id,
                                            order_queue_line):
        """
        ...
        if not listing:
            ...
            if not ebay_product and not instance.seller_id.create_new_product:
                ...
                order_queue_line.write({'state': 'failed', 'processed_at': datetime.now()})
                return True
            if instance.seller_id.create_new_product:
                try:
                    item_response = ebay_product_product_obj.call_get_items_ebay_api(instance, item_id)
                except Exception as error:
                    log_line = log_lines_obj.create_log_lines(error, log_book_id.model_id.id, False, log_book_id,
                                                              order_ref='order_ref')
                    log_line.write({'ebay_order_data_queue_line_id': order_queue_line.id})
                    order_queue_line.write({'state': 'failed', 'processed_at': datetime.now()})
                    return True
                ...
                product_queue, last_product_queue = product_data_queue_obj.create_product_queue(instance,
                                                                                                [item_response], False,
                ...
                listing_obj.create_or_update_product_with_listings(instance, item_response, log_book_id,
    ...
    def create_ebay_sale_order_lines(self, order_response, instance, sale_order, log_book_id, order_queue):
        """
    ...
    def get_ebay_shipping_details(self, instance, order_response, log_book_id, order_queue_line):
        """
        ...
        if not delivery_carrier:
            if not instance.seller_id.ebay_is_create_delivery_carrier:
                order_ref = order_response['OrderID']
                message = "Order %s skipped due to shipping %s not found in Delivery Methods. \n " \
                          "To create Shipping method automatically, \n" \
                          " please go to configurations > settings > seller > Is Delivery Carrier Create?" % (
                              order_ref, shipping_service_name)
    ...
    def ebay_update_order_status(self, instance):
        ...
        model_id = log_lines_obj.get_model_id('sale.order')
        log_book_id = log_book_obj.create_common_log_book('export', 'ebay_instance_id', instance, model_id, 'ebay_ept')
        ...
        for picking in pickings:
            ...
            if order_lines and order_lines.filtered(
                ...
                _logger.info(message)
                log_lines_obj.create_log_lines(message, log_book_id.model_id.id, False, log_book_id,
            ...
            for order_line in order_lines:
                ...
                try:
                    ...
                    log_lines_obj.create_log_lines(error, log_book_id.model_id.id, False, log_book_id,
                                                   order_ref=sale_order.name)
