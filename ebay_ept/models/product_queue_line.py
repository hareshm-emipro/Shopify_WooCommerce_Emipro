...
class SyncImportProductQueueLine(models.Model):
    ...
    instance_id = fields.Many2one(
        'ebay.instance.ept', string='Sites', help="Products imported from or Synced to this Ebay Site.")
        ...
        for item in items:
            if item:
                if not product_queue:
                    product_queue = self.ebay_create_product_queue(instance)
    ...
    def ebay_create_product_queue(self, instance):
        """
        ...
        product_queue_vals = {
            'seller_id': instance and instance.seller_id.id or False,
            'instance_id': instance and instance.id or False, 'state': 'draft'}
    ...
    def process_import_product_queue_data(self):
        ...
        if queue_id:
            log_book_id = self.create_or_update_ebay_product_queue_log(queue_id)
            self.env.cr.execute(
            ...
            for product_queue_line in self:
                ...
                product_listing_obj.sync_product_listings(product_queue_line, log_book_id)
    ...
    def create_or_update_ebay_product_queue_log(self, queue_id):
        """
        This method is used to create a logbook if the existing product queue is not a logbook.
        @author: Haresh Mori @Emipro Technologies Pvt. Ltd on date 16 December 2021 .
        Task_id:180925 - Import Products / variants
        """
        ...
        if queue_id.import_product_common_log_book_id:
            ...
            model_id = log_line_obj.get_model_id('ebay.product.listing.ept')
            log_book_id = log_book_obj.create_common_log_book('import', 'ebay_instance_id', queue_id.instance_id,
                                                              model_id, 'ebay_ept')
        return log_book_id

    def prepare_queue_line_vals(self, instance, item_id, data, product_queue, is_create_odoo_product, is_sync_stock,
                                is_sync_price):
        """
