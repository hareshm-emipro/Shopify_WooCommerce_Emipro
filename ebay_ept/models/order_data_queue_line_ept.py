...
class EbayOrderDataQueueLineEpt(models.Model):
    ...
    def process_import_order_queue_data(self):
        ...
        if queue_id:
            ...
            log_book_id = self.create_update_ebay_order_queue_log(queue_id)
            self.env.cr.execute(
            ...
            for order_queue_line in self:
                sale_order_obj.create_ebay_sales_order_ept(seller_id, order_queue_line, log_book_id, queue_id)
    ...
    def create_update_ebay_order_queue_log(self, queue_id):
        """
        Create or update eBay order queue logs
        :param queue_id: eBay order date queue object
        :return: eBay order date queue object
        """
        if queue_id.order_common_log_book_id:
            ...
            log_book_id = self.env["common.log.book.ept"].create(
                {'type': 'import', 'module': 'ebay_ept', 'model_id': model_id, 'res_id': queue_id.id,
                 'ebay_seller_id': queue_id.seller_id.id})
        return log_book_id
