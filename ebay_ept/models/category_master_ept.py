...
class EbayCategoryMasterEpt(models.Model):
    ...
    def create_or_update_ebay_category(self, categories_resp, instance, category_ids, parent_category=False):
        ...
        if parent_category:
            ...
            ebay_category = self.search_child_category(category_id, instance.id)
        if ebay_category:
    ...
    def get_attributes(self, max_name_levels, max_value_per_name):
        ...
        results = self.call_get_ebay_attributes_api(instance, max_name_levels, max_value_per_name)
        attributes = []
        recommendations = results.dict().get('Recommendations', {})
        if isinstance(recommendations.get('NameRecommendation', []), list):
            attributes = recommendations.get('NameRecommendation', [])
        else:
            attributes.append(recommendations.get('NameRecommendation', []))
        for attribute in attributes:
            category_attribute = self.search_or_create_ebay_category_attribute(attribute.get('Name'), self.id)
            attribute_values = []
            if isinstance(attribute.get('ValueRecommendation', []), list):
                attribute_values = attribute.get('ValueRecommendation', [])
            else:
                attribute_values.append(attribute.get('ValueRecommendation', {}))
            for attribute_value in attribute_values:
                self.search_or_create_ebay_category_attribute_values(attribute_value.get('Value'),
                                                                     category_attribute.id)
        return True

    def call_get_ebay_attributes_api(self, instance, max_name_levels, max_value_per_name):
        """
        Call get eBay attributes using API
        :param instance: eBay instance object
        :param max_name_levels: Max name levels
        :param max_value_per_name: Max value per name
        :return: eBay categories attributes
        Migration done by Haresh Mori @ Emipro on date 23 December 2021 .
        """
        api = instance.get_trading_api_object()
        para = {'CategoryID': self.ebay_category_id}
        if max_name_levels:
            para.update({'MaxNames': max_name_levels})
        if max_value_per_name:
            para.update({'MaxValuesPerName': max_value_per_name})
        return api.execute('GetCategorySpecifics', para)
