...
class EbayProcessImportExport(models.TransientModel):
    ...
    def ebay_sync_import_products(self, instance_ids):
        """
        ...
        @param instance_ids: Records of sites(instance)
        ...
        seller_id = instance_ids[0].seller_id
        ...
        date_range = self.prepare_date_range_for_import_product(instance_ids)
        product_queue_list = []
        for instance in instance_ids:
            existing_product_queue = False
            for from_date, to_date in date_range:
                product_result = self.sync_import_products(instance, from_date, to_date)
    ...
    def prepare_date_range_for_import_product(self, instance_ids):
        """
        ...
        """
        if self._context.get('is_call_from_cron'):
            from_date = instance_ids[0].seller_id.sync_active_products_start_date
            to_date = datetime.now().date()
            instance_ids[0].seller_id.sync_active_products_start_date = to_date
        else:
    ...
    def sync_import_products(self, instance, from_date, to_date):
    ...
    def create_products_from_file(self, product_data):
        ...
        for instance in self.instance_ids:
            log_book_id = log_book_obj.create_common_log_book('import', 'ebay_instance_id', instance, model_id,
                                                              'ebay_ept')
            for product_dic in product_data:
                ...
                if not product_dic["product_template_id"] or not product_dic["product_id"]:
                    message = "product_template_id Or product_id As Per Odoo Product in file at row " \
                              "%s " % row_no
                    log_line_obj.create_log_lines(message, model_id, False, log_book_id)
                    continue
