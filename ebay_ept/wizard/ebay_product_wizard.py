...
class EbayProductWizard(models.TransientModel):
    ...
    def product_template_update_stock_in_ebay(self):
        ...
        model_id = log_line_obj.get_model_id('ebay.product.product.ept')
        ...
        for instance in ebay_instances:
            ...
            log_book_id = log_book_obj.create_common_log_book('export', 'ebay_instance_id', instance, model_id,
                                                              'ebay_ept')
            warehouse_ids = instance.ebay_stock_warehouse_ids
            if not warehouse_ids:
                message = "No Warehouse found for Export Stock in Site: %s" % instance.name
                log_line_obj.create_log_lines(message, log_book_id.model_id.id, False, log_book_id)
                return False
            ...
            if ebay_exported_products:
                ebay_products.export_stock_levels_ebay(instance, ebay_exported_products, log_book_id,
                                                       ebay_product_stock)
        return True
    ...
    def update_product_template_price_in_ebay(self):
        ...
        ebay_product_ept = self.env[EBAY_PRODUCT_EPT]
        active_ids = self._context.get('active_ids', [])
        ebay_product_templates = ebay_product_template_obj.browse(active_ids).filtered(lambda l: l.instance_id.active
    ...
    def export_product_in_ebay(self):
        ...
        model_id = self.env['common.log.lines.ept'].get_model_id('ebay.product.template.ept')
        active_ids = self._context.get('active_ids', [])
        ...
        if ebay_product_template:
            log_book_id = self.search_create_log_book_with_res_id(model_id, ebay_product_template,
                                                                  ebay_product_template.instance_id)
            if ebay_product_template.count_total_variants == 1:
                ebay_product_tmpl_obj.export_individual_item(ebay_product_template, self.publish_in_ebay,
                                                             self.schedule_time, log_book_id)
            else:
    ...
    def relist_product_in_ebay(self):
        ...
        model_id = self.env['common.log.lines.ept'].get_model_id('ebay.product.template.ept')
        ebay_product_template = ebay_product_tmpl_obj.browse(active_ids).filtered(lambda x: x.exported_in_ebay)
        product_listing = ebay_product_template.product_listing_ids.filtered(lambda x: x.ebay_product_tmpl_id ==
                                                                                       ebay_product_template and
                                                                                       x.instance_id ==
                                                                                       ebay_product_template.instance_id
                                                                                       and x.state == 'Ended')
    ...
    def search_create_log_book_with_res_id(self, model_id, res_id, instance_id):
        """
        This method is used to search the existing logbook related to eBay template if not found then it will create
        a new logbook.
        @author: Haresh Mori @Emipro Technologies Pvt. Ltd on date 31 December 2021 .
        Task_id: 180924 - Export Products / Variants
        """
        log_book_obj = self.env["common.log.book.ept"]
        log_book_id = log_book_obj.search([("res_id", '=', res_id.id),
                                           ("ebay_instance_id", '=', instance_id.id)],
                                          limit=1)
        if not log_book_id:
            log_book_id = log_book_obj.create(
                {"type": 'export', "module": 'ebay_ept', 'ebay_instance_id': instance_id.id,
                 "model_id": model_id, "active": True, "res_id": res_id.id})
